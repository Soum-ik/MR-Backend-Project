...
import { TIME_FILTER_OPTIONS, timeFilterSchema } from "../../../utils/calculateDateRange";
...
const getVisitors = catchAsync(async (req: Request, res: Response) => {
    ...
    const { startDate, endDate } = timeFilter === TIME_FILTER_OPTIONS.ALL_TIME
        ? {
            startDate: new Date(0),
            endDate: new Date()
        }
    ...
    switch (timeFilter) {
        ...
        case TIME_FILTER_OPTIONS.THIS_MONTH:
        case TIME_FILTER_OPTIONS.LAST_MONTH:
            visitorData = await getVisitorDataByPeriod(startDate, endDate, 'day', formatDate);
            break;
        case TIME_FILTER_OPTIONS.LAST_3_MONTHS:
            visitorData = await getVisitorDataByPeriod(startDate, endDate, '15days', formatDate);
            break;
        case TIME_FILTER_OPTIONS.LAST_6_MONTHS:
        ...
        case TIME_FILTER_OPTIONS.LAST2_YEAR:
            ...
            break;
        default:
            visitorData = await getVisitorDataByPeriod(startDate, endDate, 'day');
    }
...
async function getYearlyVisitorData(startDate: Date, endDate: Date) {
    const yearsToFetch = Math.ceil((endDate.getFullYear() - startDate.getFullYear()) + 1);
    ...
    for (let i = 0; i < yearsToFetch; i++) {
        ...
        const yearData = await Promise.all(
            Array.from({ length: 12 }, (_, monthIndex) => {
                ...
                return getVisitorsForPeriod(monthStart, monthEnd, currentYear, monthIndex);
            })
        );
...
async function getVisitorsForPeriod(monthStart: Date, monthEnd: Date, year: number, monthIndex: number) {
    const [totalVisitors, newVisitors, returningVisitors] = await Promise.all([
        prisma.visitors.count({
            where: {
                createdAt: {
...
function calculatePeriods(startDate: Date, endDate: Date, type: '15days' | 'month' | 'day') {
    const periods = [];
