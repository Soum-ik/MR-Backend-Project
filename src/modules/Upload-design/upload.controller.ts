...
export const UploadDesign = async (req: Request, res: Response) => {
  try {
    ...
    const validatedData: UploadDesignSchemaInterface = uploadDesignSchema.parse(
      req.body,
    );
    ...
    if (convertStringIntoNumber) {
      ...
      specialSerialCodeGenarator = designSerialGenerator(
        convertedSerialUpdateNumber,
      );
    }
