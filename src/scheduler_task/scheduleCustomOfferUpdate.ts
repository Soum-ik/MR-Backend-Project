    ...
    try {
        ...
        const messageList = await prisma.message.findMany({
            where: {
                ...
                createdAt: {
                    lt: new Date(now.getTime() - 1 * 60 * 1000), // Subtract 48 hours in milliseconds
        ...
        if (messageList.length > 0) {
            await Promise.all(messageList.map(async (message) => {
                ...
                const { isWithdrawn, ...rest } = customOffer as unknown as CustomOfferI
                ...
                if (isCustomOfferI(customOffer)) {
                    print.yellow(
                        `Processing custom offer for message: ${uniqueId}.`
                    );
                    ...
                    try {
                        ...
                        print.red(`Error updating customOffer for message ${uniqueId}`, err);
                    }
                }
            }));
        }
